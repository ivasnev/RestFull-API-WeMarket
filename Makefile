...
postgres:
	docker stop WeMarket-postgres || true
	docker run --rm --detach --name=WeMarket-postgres \
		--env POSTGRES_USER=user \
		--env POSTGRES_PASSWORD=hackme \
		--env POSTGRES_DB=WeMarket \
		--publish 5432:5432 postgres

test: lint postgres
	env/bin/pytest -vv --cov=WeMarket --cov-report=term-missing tests
